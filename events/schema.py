...
#         if not page:
#             return active_events.order_by(func.lower(EventsModel.event_title)).all() # noqa
#         page = validate_page(page)
#         self.query_total = active_events.count()
#         result = active_events.order_by(
#             func.lower(EventsModel.event_title)).limit(
#             per_page).offset(page * per_page)
#         if result.count() == 0:
#             return GraphQLError("No more events")
#         return result
...
#     all_events_by_date_range = graphene.List(
#         PaginateEvents,
...
#         response = PaginateEvents(**kwargs)
#         return response

#     def resolve_all_events_by_date_range(self, info, start_date=None, end_date=None): # noqa
#         # get all events by date range
#         query = Events.get_query(info)
#         events = query.filter(EventsModel.state == "active")
#         events_by_range = filter_events_by_date_range(
#             events,
#             start_date, end_date
#             )
